import ReportIcon from '../../../../assets/icons/report.svg';

export const ClaimedArtistBanner = () => {
  const handleReportClick = () => {
  };

  return (
    <div className="bg-red-900/20 border border-red-500 rounded-md p-4 mb-4">
      <div className="flex items-start gap-3">
        <div className="flex-shrink-0 mt-0.5">
          <svg className="w-5 h-5 text-red-400" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
          </svg>
        </div>
        <div className="flex-grow">
          <h3 className="text-red-400 font-medium mb-2">This artist has already been claimed</h3>
          <p className="text-gray-300 text-sm mb-3">
            If you're part of the team who claimed this artist, please reach out to them. If you believe the claim is a mistake, contact our support.
          </p>
          <button
            onClick={handleReportClick}
            className="flex items-center text-sm text-red-400 hover:text-red-300 transition-colors"
          >
            <ReportIcon className="w-4 h-4 mr-2" />
            Contact Support
          </button>
        </div>
      </div>
    </div>
  );
};
